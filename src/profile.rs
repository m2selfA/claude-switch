...
impl ProfileManager {
    ...
    #[cfg(not(target_os = "windows"))]
    fn build_sh_settings(env: &LightweightEnv) -> String {
        let mut env_map = serde_json::Map::new();
    ...
    fn generate_sh_content(&self, profile: &Profile) -> Result<String> {
        let kind_label = if profile.kind == ProfileKind::Full { "full" } else { "lightweight" };
        let has_launch = profile.launch_args.as_ref().map_or(false, |a| !a.is_empty());
        let launch_str = profile.launch_args.as_ref()
            .map(|a| a.join(" "))
            .unwrap_or_default();
        ...
        let settings_var = if profile.kind == ProfileKind::Lightweight {
            profile.env.as_ref().map(|e| {
                format!("SETTINGS={}", Self::build_sh_settings(e))
            })
        } else {
            None
        };
        ...
        let settings_part = if settings_var.is_some() { " --settings \"$SETTINGS\"" } else { "" };
        let launch_part = if has_launch { &format!(" {}", launch_str) } else { "" };
